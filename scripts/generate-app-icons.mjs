// アプリアイコン／スプラッシュ生成：public/cooksync-mark.svg（ベクタ）から
// iOSネイティブ（Capacitor 8）用の画像を書き出す。
...
const src = path.join(root, "public", "cooksync-mark.svg");
...
/** SVGを指定サイズで描画してバッファ化（densityを上げないとベクタがぼやける） */
function render(size) {
  return sharp(src, { density: Math.max(72, (72 * size) / 1024) })
...
async function writeIcon(out, size) {
  const mark = await render(size);
  await sharp({
    create: { width: size, height: size, channels: 3, background: BG },
  })
    .composite([{ input: mark }])
    .removeAlpha() // アルファチャンネルを残さない（App Store Connect の検証で弾かれる）
    .png()
    .toFile(path.join(root, out));
  console.log(`OK ${out} (${size}x${size})`);
}

/** スプラッシュ＝背景色ベタ塗りの中央にマークを小さく置く（拡大でぼやけないよう余白多め） */
