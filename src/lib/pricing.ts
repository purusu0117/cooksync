 ...
 *
 * ✅ **サーバーで強制済み（2026-08-01）。** quotaServer.consume() の①bで、
 *    uidごとの月間原価カウンタ（見積もりEST_YENベース＝安全側）をこの値と比べている。
 *    無料ユーザーには掛けない（無料の週次枠フル使用は月¥296 > ¥220 で、正当な利用を
 *    止めてしまう。無料側の上限は週次枠そのものが担う）。テストは budgetGuard.test.ts。
 */
