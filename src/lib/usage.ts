...
// AI使用量の無料枠管理。原価が出るAI機能（レシピ探索/写真認識）だけメーター制にする。
...
export type AiKind = "research" | "scan";
...
export function useUsage() {
  ...
  function remaining(kind: AiKind): number {
    ...
    return Math.max(0, FREE_LIMITS[kind] - (rec[kind] ?? 0));
  }
  function canUse(kind: AiKind): boolean {
    return premium || (rec[kind] ?? 0) < FREE_LIMITS[kind];
  }
