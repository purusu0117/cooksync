...
function buildPrompt(b: ResearchBody): string {
  ...
  return [
    ...
    "- 【sourcesは必須】実在のレシピを必ずWeb検索で参照し、sources に **最低1件、できれば2件** 入れる。label（サイト名＋レシピ名）・url（実在するURL）・popularity（つくれぽ数/再生数などの人気の根拠）をセットで書く。クックパッド/白ごはん.com/リュウジのバズレシピ/つくおき/Nadia などの実ページを挙げる。出典のない創作は禁止。sourcesが空の候補は出力しないこと。",
    shopMode === "stock"
